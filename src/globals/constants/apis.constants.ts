...
const baseApi =
	MODE === "dev" ? "http://localhost:8080/" : "https://54.234.87.11/";

const addBaseApi = (path: string) => baseApi;
const FormApi = (path: string) => "/forms" + path;
const PagesApi = (path: string) => "/pages" + path;
...
const apis = {
	...
	checkCategoryNameisValid: (val: string) =>
		FormApi(`/createCategory/getIsCategoryNameValid/${val}`),
	createCategory: FormApi(`/createCategory/createCategory`),
	getEditFormData: (id: string) =>
		FormApi(`/createCategory/getEditFormData/${id}`),
	editCategory: FormApi(`/createCategory/editCategory`),
	getCategoryUnique: (name: string) =>
		FormApi(`createCategory/getCategoryUnique/${name}`),
	...
	getCompanyNameUnique: (name: string) =>
		FormApi(`/createCompany/getCompanyUnique/${name}`),
	checkCompanyProductNameUnique: (
		company_name: string,
		category_name: string,
		item_name: string
	) =>
		FormApi(
			`/createCompanyProduct/checkCompanyProductUnique/${company_name}/${category_name}/${item_name}`
		),
	...
	getItemsOfCategory: (id: string) =>
		FormApi(`/createCompanyProduct/getItemsofCategory/${id}`),
	getFormdescription: (d: RequestQuery) =>
		FormApi(
			`/createCompanyProduct/getFormDescription?companyId=${d.companyId}&itemId=${d.itemId}&categoryId=${d.categoryId}`
		),
	editFormDescription: (id: string) =>
		FormApi(`/createCompanyProduct/editFormDescription/${id}`),
	editCompanyProduct: FormApi(`/createCompanyProduct/editCompanyProduct`),
	createItem: FormApi(`/createItem/createItem`),
	getDescriptionLables: (id: string) =>
		FormApi(`/createItem/getDescriptionLables/${id}`),
	getEditFormSpec: (id: string) => FormApi(`/createItem/getEditFormSpec/${id}`),
	editItem: FormApi(`/createItem/editItem`),
	getItemUnique: (item_name: string, category_name: string) =>
		FormApi(`createItem/getItemUnique/${item_name}/${category_name}`),
	...
	updateItemActiveCompany: PagesApi(
		`/categoryNumberOfItem/updateItemActiveCompany`
	),
	getActiveInactiveCompany: (id: string) =>
		PagesApi(`/categoryNumberOfItem/getActiveInactiveCompany/${id}`),
	...
	getProductGridData: (id: string) =>
		PagesApi(`/companySpecification/getProductGridData/${id}`),
	getAllCompaniesWithBaseRate: PagesApi(
		`/setBasePrice/getAllCompaniesWithBaseRate`
	),
	updateCompanyBaseRate: PagesApi(`/setBasePrice/updateCompanyBaseRate`),
	getProductSpecification: (id: string) =>
		PagesApi(`/productSpecification/getProductSpecification/${id}`),
	getFilteredProductsPriceStructure: PagesApi(
		`/companyProductListing/getFilteredProductsPriceStructure`
	),
	updateAllProductPriceStructureValue: PagesApi(
		`/companyProductListing/updateAllProductPriceStructureValue`
	),
};
